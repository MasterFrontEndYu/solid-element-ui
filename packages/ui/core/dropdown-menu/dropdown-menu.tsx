      ...
      {(config) => (
        <Show when={!config.separator} fallback={<KDropdownMenu.Separator class={defaultClass.separator} />}>
          ...
          >
            {/* 子菜单渲染逻辑 */}
            <KDropdownMenu.Sub>
              <KDropdownMenu.SubTrigger class={defaultClass.item}>
                {config.label}
                <ChevronRight class={defaultClass.subIcon} />
              </KDropdownMenu.SubTrigger>
              <KDropdownMenu.Portal>
                <KDropdownMenu.SubContent class={defaultClass.content}>
...
export const DropdownMenu = (props: DropdownMenuProps) => {
  return (
    <KDropdownMenu placement={props.placement ?? "bottom-start"}>
      <KDropdownMenu.Trigger as="div" class={defaultClass.trigger}>
        {props.trigger}
      </KDropdownMenu.Trigger>

      <KDropdownMenu.Portal>
        <KDropdownMenu.Content class={defaultClass.content}>
