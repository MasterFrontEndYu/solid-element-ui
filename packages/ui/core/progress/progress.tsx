  ...
  return (
    <KProgress class={cn('flex flex-col gap-2 w-full antialiased', props.class)} {...others}>
      <Show when={props.label || props.showValue}>
        <div class={cn('flex justify-between items-center text-sm font-medium text-slate-700 dark:text-slate-300', props.labelContainerClass)}>
      ...
      <KProgress.Track class={cn('h-2 w-full rounded-full bg-appoverflow-hidden', props.trackClass)}>
        <KProgress.Fill
          class={cn('h-full bg-primary transition-all duration-300 ease-in-out data-[indeterminate]:animate-progress-indeterminate', props.fillClass)}
