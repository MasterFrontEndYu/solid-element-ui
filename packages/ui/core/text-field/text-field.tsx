  ...
  return (
    ...
    >
      <Show when={props.label}>
        <KTextField.Label class={cn('text-sm font-medium text-muted peer-disabled:cursor-not-allowed peer-disabled:opacity-70', props.labelClass)}>
          {props.label}
        </KTextField.Label>
      </Show>

      <KTextField.Input
        class={cn('flex h-10 w-full rounded-md border border-light bg-app px-3 py-2 text-sm transition-shadow text-main ring-offset-white file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted/40 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary disabled:cursor-not-allowed disabled:opacity-50 data-[invalid]:border-danger data-[invalid]:focus-visible:ring-danger', props.inputClass)}
        type={props.type}
      ...
      <Show when={props.errorMessage}>
        <KTextField.ErrorMessage class={cn('text-xs text-danger animate-in fade-in-50 slide-in-from-top-1', props.errorMessageClass)}>
          {props.errorMessage}
